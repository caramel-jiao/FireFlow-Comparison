...
def main(
    ...
    """
    data_json = read_json(os.path.join(args.data_root, 'mapping_file.json'))
    ...
    for itm_name, itm_data in tqdm(data_json.items()):

        img_path = os.path.join(args.data_root, 'annotation_images', itm_data['image_path'])
        synthesis_prompt = itm_data['original_prompt'].replace('[', '').replace(']', '')
        edit_prompt = itm_data['editing_prompt'].replace('[', '').replace(']', '')
        edit_name = itm_data['editing_instruction'].replace(' ', '_')
        ...
        while opts is not None:
            ...
            for x in batch_x:
                ...
                img.save(os.path.join(base_folder, itm_name + '_' + edit_name + '.png'))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', default='output_editing', type=str,
    ...
    parser.add_argument('--data_root', default='/home/jovyan/shared/jiaoguanlong/shared-datasets/PIE_Bench/PIE-Bench_v1', type=str,
                        help='root of evaluation dataset (PIE-Bench)')
